...
const isUserPresent = async ({ formData }) => {
    const url = server.isUserExist
    return await axios.post(url, formData).then(data => {
        return data.data
    }).catch(er => {
        return null
    })
...
export { loginService, registerService, accesstokenLogin, searchUser, singleUserData, isUserPresent }